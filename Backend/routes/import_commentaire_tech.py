...
def _extract_palier_from_evenements(evenements: str | None) -> str | None:
    ...
    if (
        ("aucun" in low or "aucune" in low or "aucunes" in low)
        and ("regle" in low or "règle" in low)
        and ("applic" in low)
    ):
        return "PALIER_1"
    return None
...
async def import_commentaire_tech_cr10(
    ...
    # Dédoublonnage par (ot_key, user_id) — identique au pattern de imports.py
    ...
    for raw in reader:
        ...
        # Dernière ligne gagne pour la même clé dans le fichier
        by_key[ot_key] = {
            "id_externe": ot_key,
            "evenements": evenements,
            "palier": palier,
            "compte_rendu": compte_rendu,
            "user_id": current_user.id,  # ← isolation stricte par utilisateur
        }
    ...
    for row in by_key.values():
        if row.get("evenements"):
            with_evenements += 1
        if row.get("palier"):
            with_palier += 1
        if row.get("compte_rendu"):
            with_compte_rendu += 1

    rows_list = list(by_key.values())

    # ── pg_insert avec index_elements composite (id_externe, user_id) ──────────
    # Identique au pattern imports.py — garantit l'isolation par user_id
    t = RawPraxedoCr10.__table__
    stmt = pg_insert(t).values(rows_list)
    stmt = stmt.on_conflict_do_update(
        index_elements=[t.c.id_externe, t.c.user_id],  # ← PK composite obligatoire
        set_={
